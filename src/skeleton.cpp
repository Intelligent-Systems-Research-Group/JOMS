...
namespace hyp {
	...
	void Skeleton::calculate_kinematic_chain(std::vector<Vector3>& joint_positions,
	    ...
	    bool isAbsolute = resJoints.cols() != 0;
			...
			for (size_t i = 0; i < jointCount; i++) {
				...
				Matrix4 relative;
				Vector3 joint_rel;
				if (i == 0) joint_rel = joint_positions[0];
				else {
					std::string ancestor = this->ancestors[part_names[i]];
					int j = std::find(part_names.begin(), part_names.end(), ancestor) - part_names.begin();
				...
				Matrix4 fixedJointTransform; fixedJointTransform.setIdentity();
			...
			for (size_t i = 1; i < jointCount; i++) {
				...
				if(isAbsolute) {
					...
					//absolutes[i].block<3,1>(0,3) = absolutes[j].block<3,3>(0,0) * relatives[i].block<3,1>(0,3) 
		...
		void Skeleton::set_joint_location(std::string from, std::string to, Vector3 position, std::vector<Vector3>& joint_positions) {
			assert(ancestors[to] == from || ancestors[from] == to);
			std::string joint_name = ancestors[to] == from ? to : from;
			int index = std::find(part_names.begin(), part_names.end(), joint_name) - part_names.begin();
			joint_positions[index] = position;
			//std::cout << index <<  ":" << joint_name << " -> " << joint_positions[index].transpose() << std::endl;
		}
	...
	for(std::string name : rigid)
		...
		std::pair<std::string, std::string> Skeleton::getJoint(size_t index) {
			return std::pair<std::string, std::string>(part_names[index], ancestors[part_names[index]]);
		}

		size_t Skeleton::getParentJoint(size_t index) {
			auto parentName = ancestors[part_names[index]];
			return std::find(part_names.begin(), part_names.end(), parentName) - part_names.begin();
		}
		...
		int Skeleton::getJointIndex(std::string from, std::string to) {
			...
			if (itera != ancestors.end() && itera->second == to) {
				for (size_t i = 0; i < part_names.size(); i++) {
					if (itera->first == part_names[i]) return i;
				}
			}
			else if (iterb != ancestors.end() && iterb->second == from) {
				for (size_t i = 0; i < part_names.size(); i++) {
					if (iterb->first == part_names[i]) return i;
				}
			}
		...
		size_t Skeleton::getJointCount() {
			return part_names.size();
		}
		/*
		void print() {
		...
		*/
		...
		std::vector<Scalar> Skeleton::extractVertexWeights(const Weights& weights, size_t index) {
			...
			for (size_t i = 0; i < part_names.size(); i++) {
				...
				if (iter != weights.end()) {
					//std::cout << pair.first << " " << pair.second << " " << iter->second << std::endl;
		...
		void Skeleton::setPose(std::vector<Matrix3>& vec_theta, std::map<std::pair<std::string, std::string>,
			Vector3>& joint_locations, std::vector<Matrix4>& absolutes) {
			//std::vector<DMatrix4> absolutes(part_names.size());
			std::vector<Vector3> joint_positions(part_names.size());
			for (auto it_joint = joint_locations.begin(); it_joint != joint_locations.end(); ++it_joint) {
				//std::cout << it_joint->first.first << " <-> " << it_joint->first.second << " : " << it_joint->second.transpose() << endl;
				set_joint_location(it_joint->first.first, it_joint->first.second, it_joint->second, joint_positions);
			}
			calculate_kinematic_chain(joint_positions, vec_theta, absolutes);
		}
		Vector3 Skeleton::skin_vertex(std::vector<Matrix4>& absolutes, const VectorX& weights, const Vector3& position) {
	...
	Vector3 Skeleton::rigidOrientation(int j) {
		Vector3 rot; rot.setZero();
        Matrix3X angle(3,1);
        Matrix3X euler(3,1); euler.setZero();
		return euler;
		Matrix3 R,R2,Q;
		Matrix3X angle2(3,1);
