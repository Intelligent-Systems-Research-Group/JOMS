...
struct MeshTopology {
	...
	void vertexAdjacency(std::map<size_t, std::set<size_t>>& adjacency, IMatrix4X* faceAdj = NULL);
...
bool loadObj(std::string path, MeshTopology* mesh, Matrix3X* verts, Matrix3X* normals = NULL, 
	double scale = 1.0, bool mirror = false, bool rot = false);
bool saveObj(std::string path, const MeshTopology* mesh, const Matrix3X* verts);
void dumpCloud(std::string name, int iter, const Matrix3X& cloud, const Matrix3X* normals = NULL);
bool readCloud(std::string name, Matrix3X* cloud, Matrix3X* normals,double scale, bool mirror, bool rot=false);
