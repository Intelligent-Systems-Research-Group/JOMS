...
void Model::write(std::string path) {
	...
	int npcs = pcs.size();
	MatrixX betas; betas.resize(npersons,npcs);
	...
	Matrix3X ts; ts.resize(3,scans.size());
	MatrixX Rs; Rs.resize(3*njoints,scans.size());
...
void Model::read(std::string path) {
	...
	assert(scans[0].posebase.cols() == Rs.rows()/3 && "pose joints != Rs.rows()/3");
...
void Model::init(const Matrix3X& _mean, const Matrix3X& _joints,
                const MatrixX& _weights, 
				int ncomp, int ndefcomp, int npersons, int nscans) {
