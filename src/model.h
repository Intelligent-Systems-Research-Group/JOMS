...
struct Model {
    ...
    */
	...
	void init(const Matrix3X& _mean, const Matrix3X& _joints,
				const MatrixX& _weights, int ncomp, int ndefcomp,
				int nperson, int nscans);
};
...
public:
	...
	BarrierType type;
public:
	BarrierIndex(BarrierType _type=NONE, int _index=0) : type(_type), index(_index) {}
	...
	bool hasType(BarrierType _type) const{
		return _type == type;
	}
	
	bool sharesType(const BarrierIndex& bi) const{
		return type == bi.type;
	}
