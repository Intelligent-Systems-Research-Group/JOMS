...
static FbxNode* createBaseMesh(FbxScene* pScene, const char* pName, Model* model, int personId);
static FbxNode* createJointMesh(FbxScene* pScene, const char* pName, FbxAnimLayer* myAnimBaseLayer, Model* model, int personId);
static FbxNode* createSkeleton(FbxScene* pScene, const char* pName, FbxAnimLayer* myAnimBaseLayer, int* mode, FbxNode* pMeshNode, Model* model, FbxNode** bones, int personId, int poseId, int endId);
static void LinkPatchToSkeleton(FbxScene* pScene, FbxNode* pPatch, FbxNode* pSkeletonRoot, Model* model, FbxNode** bones);
...
bool writePersonFbx(std::string path, int* mode, Model* model, int personId, int poseId, int endId) {
	...
	FbxNode* lJoint = createJointMesh(lScene, "joints", myAnimBaseLayer, model, personId);
	base->AddChild(lJoint);
	FbxNode* skel = createSkeleton(lScene, "", myAnimBaseLayer, mode, lCube, model, bones, personId, poseId, endId);
	base->AddChild(skel);
	...
	std::string outpath = path + ".fbx";
        //std::string outpath = path + (char)('X' + mode[0]) + (char)('X' + mode[1]) + (char)('X' + mode[2]) + ".fbx";
	//std::string outpath2 = path + "bin" + (char)('X' + mode[0]) + (char)('X' + mode[1]) + (char)('X' + mode[2]) + ".fbx";
	//SaveScene(lSdkManager, lScene, outpath.c_str(), -1);
...
//#ifdef POSE_DEFORM
                ...
                //weight = .25;
//#else
//				double weight = global->initialSkinWeights(k, j);
//#endif
...
static FbxNode* createSkeleton(FbxScene* pScene, const char* pName, FbxAnimLayer* myAnimBaseLayer, int* mode, FbxNode* pMeshNode, Model* model, FbxNode** bones, int personId, int poseId, int endId) {
	...
	//std::cout << "max: " << jointPos.maxCoeff() << " min: " << jointPos.minCoeff() << std::endl;
	...
	for (int i = 0; i < skel->getJointCount(); i++) {
		...
		*/
		...
		bones[i]->SetRotationOrder(FbxNode::EPivotSet::eDestinationPivot, FbxEuler::eOrderXYZ);
	}

    bones[0]->LclTranslation.Set(FbxVector4(jointPos(0,0), jointPos(1,0), jointPos(2,0)));
...

	{
		...
		//auto pair = skel->getJoint(i);
		FbxAnimCurveNode* myAnimCurveNode = bones[0]->LclTranslation.GetCurveNode(myAnimBaseLayer, true);

		FbxAnimCurve* myTransCurveX = bones[0]->LclTranslation.GetCurve(myAnimBaseLayer, FBXSDK_CURVENODE_COMPONENT_X, true);
		FbxAnimCurve* myTransCurveY = bones[0]->LclTranslation.GetCurve(myAnimBaseLayer, FBXSDK_CURVENODE_COMPONENT_Y, true);
		FbxAnimCurve* myTransCurveZ = bones[0]->LclTranslation.GetCurve(myAnimBaseLayer, FBXSDK_CURVENODE_COMPONENT_Z, true);
		...
		myTransCurveX->KeySetInterpolation(myKeyIndex, FbxAnimCurveDef::eInterpolationLinear);
		myTransCurveY->KeySetInterpolation(myKeyIndex, FbxAnimCurveDef::eInterpolationLinear);
		myTransCurveZ->KeySetInterpolation(myKeyIndex, FbxAnimCurveDef::eInterpolationLinear);

		for (int t = poseId; t < endId; t++) { //fix person index model->persons[s]->scans.size()
	...
	for (int i = 0; i < skel->getJointCount(); i++) {
		auto pair = skel->getJoint(i);
		FbxAnimCurveNode* myAnimCurveNode = bones[i]->LclRotation.GetCurveNode(myAnimBaseLayer, true);

		FbxAnimCurve* myRotCurveX = bones[i]->LclRotation.GetCurve(myAnimBaseLayer, FBXSDK_CURVENODE_COMPONENT_X, true);
		FbxAnimCurve* myRotCurveY = bones[i]->LclRotation.GetCurve(myAnimBaseLayer, FBXSDK_CURVENODE_COMPONENT_Y, true);
		FbxAnimCurve* myRotCurveZ = bones[i]->LclRotation.GetCurve(myAnimBaseLayer, FBXSDK_CURVENODE_COMPONENT_Z, true);
		...
		myRotCurveX->KeySetInterpolation(myKeyIndex, FbxAnimCurveDef::eInterpolationLinear);
		myRotCurveY->KeySetInterpolation(myKeyIndex, FbxAnimCurveDef::eInterpolationLinear);
		myRotCurveZ->KeySetInterpolation(myKeyIndex, FbxAnimCurveDef::eInterpolationLinear);

		for (int t = poseId; t < endId; t++) { //fix scan size global->persons[s]->scans.size()
            /*
			Matrix3 rot = model->scans[t].posebase.col(i);
			if (t == 0) {
				std::cout << i << " : " <<  model->skeleton->getJoint(i).second <<
					" -> " << model->skeleton->getJoint(i).first << " : \n";
				std::cout <<  ":" << rot << std::endl;
			}
			//rot.transposeInPlace();
			auto euler = 180.0 / 3.141 * rot.eulerAngles(mode[0], mode[1], mode[2]);
            */
...
FbxNode* createJointMesh(FbxScene* pScene, const char* pName, FbxAnimLayer* myAnimBaseLayer, Model* model, int personId) {
    FbxMesh* lMesh = FbxMesh::Create(pScene, pName);
	
	FbxNode* lNode = FbxNode::Create(pScene, pName);


	//std::cout << "max: " << poseRegressor.maxCoeff() << " min: " << poseRegressor.minCoeff() << std::endl;
...
/*
FbxNode* createJointMesh(FbxScene* pScene, char* pName, FbxAnimLayer* myAnimBaseLayer, Model* model) {
	FbxMesh* lMesh = FbxMesh::Create(pScene, pName);
	FbxNode* lNode = FbxNode::Create(pScene, pName);
	...
	for (int i = 1; i < 16; i++) {
		jointPos(0, i) = global->jointRegressor(3 * (i - 1) + 0, 0).getValue();
		jointPos(1, i) = global->jointRegressor(3 * (i - 1) + 1, 0).getValue();
		jointPos(2, i) = global->jointRegressor(3 * (i - 1) + 2, 0).getValue();
	}
	...
	for (int i = 0; i < jointPos.cols(); i++) {
		...
		lMeanPoints[i] = FbxVector4(jointPos(0, i), jointPos(1, i), jointPos(2, i));
		//lMeanPoints[i] = FbxVector4(0,0,0);
	}

	

	FbxAnimCurveNode* myAnimCurveNode = lBlendShapeChannel->DeformPercent.GetCurveNode(myAnimBaseLayer);

	FbxAnimCurve* myScaleCurve = lBlendShapeChannel->DeformPercent.GetCurve(myAnimBaseLayer, true);
	...
	myScaleCurve->KeySetInterpolation(myKeyIndex, FbxAnimCurveDef::eInterpolationLinear);


	for (int s = 0; s < global->persons.size(); s++) {
		for (int t = 0; t < global->persons[s]->scans.size(); t++) {

			double scale = global->persons[s]->scale(0, 0).getValue();
			//rot.transposeInPlace();

			myTime.SetSecondDouble((myKeyIndex + 1)*1.0);
			myKeyIndex = myScaleCurve->KeyAdd(myTime);

			myScaleCurve->KeySetValue(myKeyIndex, 100 * SCALAR*(scale));
	...
	for (int i = 0; i < global->component_count; i++) {
		...
		FbxAnimCurve* myCompCurve = lBlendShapeChannel->DeformPercent.GetCurve(myAnimBaseLayer, true);
		...
		myCompCurve->KeySetInterpolation(myKeyIndex, FbxAnimCurveDef::eInterpolationLinear);
		...
		for (int j = 0; j < global->joint_count; j++) {
			pts[1 + j] = FbxVector4(
				SCALAR * global->jointRegressor(3 * j + 0, 1 + i).getValue(),
				SCALAR * global->jointRegressor(3 * j + 1, 1 + i).getValue(),
				SCALAR * global->jointRegressor(3 * j + 2, 1 + i).getValue()
			);
		}

		for (int s = 0; s < global->persons.size(); s++) {
			double coeff = global->persons[s]->blendCoefficient(0, i).getValue();
			for (int t = 0; t < global->persons[s]->scans.size(); t++) {

				myTime.SetSecondDouble((myKeyIndex + 1)*1.0);
				myKeyIndex = myCompCurve->KeyAdd(myTime);

				myCompCurve->KeySetValue(myKeyIndex, 100 * coeff / SCALAR);
...
static FbxNode* createBaseMesh(FbxScene* pScene, const char* pName, Model* model, int personId) {
	...
	for (int i = 9; i < 9 * (model->joints.cols()) && model->deform.size() != 0; i++) {
		int jointIdx = i / 9;
		std::stringstream buffer;
		...
		for (int j = 0; j < a.cols(); j++) {
			...
			if (regIdx_ptr == &influence[0]+2) {
				...
				if (j == 0) {
					...
					//if (i % 9 == 0) {
					//	std::cout << blendName << " : " << model->skeleton->getJoint(jointIdx).second <<
		...
		//std::cout << i << " : "  << pts[0][0] << ", " << pts[0][1] << ", " << pts[0][2] << std::endl;
		FbxBlendShape* lBlendShape = FbxBlendShape::Create(pScene, "");
		FbxBlendShapeChannel* lBlendShapeChannel = FbxBlendShapeChannel::Create(pScene, "");
		lMesh->AddDeformer(lBlendShape);
		lBlendShape->AddBlendShapeChannel(lBlendShapeChannel);
...
/*
static void dumpSubiv(std::string name, SubdivEvaluator* eval, const Matrix3X& vertices) {
	MeshTopology resultTopology;
	Matrix3X resultVertices;
	eval->generate_refined_mesh(vertices, 1, &resultTopology, &resultVertices);
	auto path = name + std::string(".obj");
	saveObj(path, &resultTopology, &resultVertices);
}

static void dumpMesh(std::string name, MeshTopology& topology, const Matrix3X& vertices) {
	auto path = name + std::string(".obj");
	saveObj(path, &topology, &vertices);
}

static void dumpCloud(std::string name, int iter, const Matrix3X& cloud, const Matrix3X* normals = NULL) {
...
bool reparameterize(const Model& source, Model* target) {
    MatrixX shapes; shapes.resize(source.scans.size(),3*source.mean.cols() + 3*source.joints.cols());
