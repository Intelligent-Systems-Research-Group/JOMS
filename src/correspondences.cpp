...
SurfaceState::SingleState::SingleState(int _index, SubdivEvaluator *eval,
                                       std::shared_ptr<RayTracer> _rayTracer,
                                       MeshTopology *mesh, Matrix3X *scan,
...
int SurfaceState::SingleState::move(Vector2 u, int f, Vector2 d, Vector2 &out,
                                    int &fout, Scalar *distance,
                                    std::vector<Vector3> *path) {

  Vector2 dir;
  // std::cout << u.transpose() << "->" << d.transpose() << std::endl;
  int jumps = eval->increment_u_crossing_edges(*mesh, localPts, f, u, d, &fout,
                                               &out, &dir, distance, path);
  // int jumps = eval->increment_u_crossing_edges(*mesh, pts, f, u, d, &fout,
  // &out, &dir, distance, path);

  // int jumps = eval->increment_accurate(*mesh, pts, f, u, d, &fout, &out,
  // &dir, distance, path); int jumps = eval->increment_u_crossing_edges(*mesh,
  // pts, f, u, d, &fout, &out, &dir, distance, path);
  return jumps;
}
...
Vector3 SurfaceState::SingleState::moveTangent(Vector2 u, int f, Vector2 d) {
  ...
  eval->evaluateSubdivSurface(pts, sps, &outS, NULL, NULL, NULL, &out_Su,
                              &out_Sv);
...
void SurfaceState::SingleState::readModelPoints(Matrix3X &outS, Matrix3X &n) {
  ...
  true);
*/
  eval->evalPointsPartial(localPts, eval->controlVertexPtex, &outS, &out_Su,
                          &out_Sv);
...
Scalar SurfaceState::SingleState::evaluateCurrent(int dataIndex, Vector2 u,
                                                  int face) {
#ifndef USE_NORMAL
  Vector3 p;
  Matrix3X outS(3, 1);
  ...
  std::vector<SurfacePoint> sps;
  sps.push_back(sp);
  eval->evaluateSubdivSurface(pts, sps, &outS);
  ...
  */
  ...
  if (shooting) {
    ...
    return (nw * (n - scanNormal)).squaredNorm() +
           (w * (p - scan->col(dataIndex))).squaredNorm();
  }
...
void SurfaceState::SingleState::setCurrentModel(
    Matrix3X &pts, Matrix3X &localPts, const std::vector<int> &excludedVertices,
    bool rebuild_tree) {
  ...
  // HACK: move masked sample points far away. This way, they will never be
  // nearest neighbors
  for (int i = 0; i < npts; i++) {
  ...
  nanoflann::SearchParams(10)); std::cout << ret_index <<
...
void SurfaceState::SingleState::alternatingUpdate(int dataIndex, Vector2 u,
                                                  int f, Vector2 d,
                                                  Vector2 &out, int &fout) {
...
bool SurfaceState::SingleState::discreteUpdate(int dataIndex, Vector2 &out,
                                               int &fout) {
  // shooting = false;

  Vector3 query_pt = scan->col(dataIndex);
#ifdef USE_NORMAL
  Vector3 query_normal;
  query_normal.setZero();
  ...
  if (shooting) {
    ...
    if (ftemp >= 0 && this->modelFaceMask[ftemp]) {
      assert(out(0, 0) >= 0.0 && out(0, 0) <= 1.0 && out(1, 0) >= 0.0 &&
             out(1, 0) <= 1.0);
      assert(utemp(0, 0) >= 0.0 && utemp(0, 0) <= 1.0 && utemp(1, 0) >= 0.0 &&
             utemp(1, 0) <= 1.0);
  ...
  //	std::cout << "knn distance deviates from computed distance!" <<
  //std::endl;
  //}
...
void SurfaceState::SingleState::disableRayTracing() { shooting = false; }

SurfaceState::SurfaceState() {}
void SurfaceState::addData(SubdivEvaluator *eval,
                           std::shared_ptr<RayTracer> _rayTracer,
                           MeshTopology *mesh, Matrix3X *scan,
                           Matrix3X *scan_normals) {
  states.push_back(SingleState(states.size() - 1, eval, _rayTracer, mesh, scan,
                               scan_normals));
}
int SurfaceState::move(int stateId, Vector2 u, int f, Vector2 d, Vector2 &out,
                       int &fout, Scalar *distance,
                       std::vector<Vector3> *path) {
  return states[stateId].move(u, f, d, out, fout, distance, path);
}

void SurfaceState::setCurrentModel(int stateId, Matrix3X &pts,
                                   Matrix3X &localPts,
                                   const std::vector<int> &excludedVertices,
                                   bool rebuild_tree) {
  // std::cout << "Set Model " << stateId << std::endl;
  states[stateId].setCurrentModel(pts, localPts, excludedVertices,
                                  rebuild_tree);
}

void SurfaceState::alternatingUpdate(int stateId, int dataIndex, Vector2 u,
                                     int f, Vector2 d, Vector2 &out,
                                     int &fout) {
  states[stateId].alternatingUpdate(dataIndex, u, f, d, out, fout);
}

bool SurfaceState::discreteUpdate(int stateId, int dataIndex, Vector2 &out,
                                  int &fout) {
