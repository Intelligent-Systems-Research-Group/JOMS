...
SubdivEvaluator::SubdivEvaluator(SubdivEvaluator const &that) { *this = that; }
...
SubdivEvaluator::GetLocalPointStencilTable() {
  return patchTable->GetLocalPointStencilTable();
}

Matrix3X SubdivEvaluator::localPoints(const Matrix3X &base) {
  // std::cout << "Local Point Computation: " << nRefinerVertices << ", " <<
  // nLocalPoints
  //    << ", " << patchTable->GetLocalPointStencilTable()->GetNumStencils() <<
  //    std::endl;
  ...
  */
  ...
  patchTable->ComputeLocalPointValues(
      &evaluation_verts_buffer[0], &evaluation_verts_buffer[nRefinerVertices]);
...
void SubdivEvaluator::init(Matrix3X const &vert_coords, Matrix3X *samples) {
  ...
  // OpenSubdiv::Far::TopologyRefiner *refiner = Refinery::Create(desc,
  // Refinery::Options(type, options));
  ...
  evaluateSubdivSurface(vert_coords, boundaries, &out_S);
  ...
  if (samples != NULL)
    samples->resize(3, controlVertexPtex.size());
  evaluateSubdivSurface(vert_coords, controlVertexPtex, samples);
...
void SubdivEvaluator::generate_refined_mesh(Matrix3X const &vert_coords,
                                            int levels, MeshTopology *mesh_out,
                                            Matrix3X *verts_out) {
  if (levels > maxlevel) {
...
void SubdivEvaluator::evaluatePtex(const Matrix3X &vert_coords, Matrix3X *out_S,
                                   triplets_t *out_dSdX, triplets_t *out_dSudX,
                                   triplets_t *out_dSvdX, Matrix3X *out_Su,
                                   Matrix3X *out_Sv, bool printDebug) const {
  evaluateSubdivSurface(vert_coords, controlVertexPtex, out_S, out_dSdX,
                        out_dSudX, out_dSvdX, out_Su, out_Sv, NULL, NULL, NULL,
                        NULL, NULL, NULL, printDebug);
}

/*
void SubdivEvaluator::move(const MeshTopology& mesh, const Matrix3X&
vert_coords, const Matrix2X& du, Matrix2X* dir) { Matrix3X out_S(3,
controlVertexPtex.size()); int nhops = 0; int loopers = 0; int totalhops = 0;
        evaluateSubdivSurface(vert_coords, controlVertexPtex, &out_S);
        for (int i = 0; i < controlVertexPtex.size(); i++) {
//controlVertexPtex.size() int face = controlVertexPtex[i].face; int newFace;
                Vector2 new_u;
                Vector2 idir; idir.setConstant(-100);
                int nhops = increment_u_crossing_edges(mesh, vert_coords, face,
controlVertexPtex[i].u, du.col(i), &newFace, &new_u, &idir); dir->col(i) = idir;
                if (nhops < 0)
                        ++loopers;
        ...
        if (loopers > 0)
                std::cerr << "[" << totalhops / Scalar(controlVertexPtex.size())
<< " hops, " << loopers << " points looped]"; else if (totalhops > 0) std::cerr
<< "[" << totalhops << "/" << Scalar(controlVertexPtex.size()) << " hops]";

}
*/
int SubdivEvaluator::increment_u_crossing_edges(
    const MeshTopology &mesh, Matrix3X const &X, int face, const Vector2 &u,
    const Vector2 &du, int *new_face_out, Vector2 *new_u_out, Vector2 *dir,
  ...
  */
  ...
  for (int count = 0;; ++count) {
    bool crossing =
        (u1_new < 0.f) || (u1_new > 1.f) || (u2_new < 0.f) || (u2_new > 1.f);
    ...
    if (!crossing) {
      ...
      *new_u_out << u1_new, u2_new;
      ...
      // std::cout << (*new_face_out) << " -> " << new_u_out->transpose()  <<
      // std::endl;

      assert((*new_u_out)[0] >= 0 && (*new_u_out)[1] <= 1.0 &&
             (*new_u_out)[1] >= 0 && (*new_u_out)[1] <= 1.0);
      return count;
    }

    // Find the new face	and the coordinates of the crossing point within the
    // old face and the new face
    int face_new;
    ...
    if ((u1_new > 1.f) && (!face_found)) {
      ...
      const Scalar u2t = u2_old + du2 * dif / du1;
      if ((u2t >= 0.f) && (u2t <= 1.f)) {
        // face_new = adjacency[face].adjface(1); aux = 1.f - u2t; face_found =
        // true;
        face_new = mesh.face_adj(1, face);
        aux = 1.f - u2t;
        face_found = true;
        u1_cross = 1.f;
        u2_cross = u2t;
    ...
    if ((u2_new < 0.f) && (!face_found)) {
      ...
      if ((u1t >= 0.f) && (u1t <= 1.f)) {
        // face_new = adjacency[face].adjface(0); aux = 1.f - u1t; face_found =
        // true;
        face_new = mesh.face_adj(0, face);
        aux = 1.f - u1t;
        face_found = true;
        u1_cross = u1t;
        u2_cross = 0.f;
    ...
    // Find the coordinates of the crossing point as part of the new face, and
    // update u_old (as that will be new u in next iter).
    unsigned int conf = 1000000;
    for (unsigned int f = 0; f < 4; f++)
      if (mesh.face_adj(f, face_new) == face) {
        conf = f;
      }
    ...
    // Evaluate the subdivision surface at the edge (with respect to the
    // original face)
    std::vector<SurfacePoint> pts;
    ...
    // *distance += (pcurrent-S.col Matrix3X localPoints(const Matrix3X&
    // base);(0)).norm();
    pcurrent = S.col(0);
    ...
    if (count == MAX_FIX_HOPS) {
      ...
      *new_face_out = face;
      ...
      // std::cerr << "Problem!!! Many jumps between the mesh faces for the
      // update of one correspondence. I remove the remaining u_increment!\n";
      return -count;
...
int SubdivEvaluator::increment_u_crossing_edges_simple(
    const MeshTopology &mesh, Matrix3X const &X, int face, const Vector2 &u,
    const Vector2 &du, int *new_face_out, Vector2 *new_u_out, Vector2 *dir,
    Scalar *distance, std::vector<Vector3> *path) {
  const int MAX_HOPS = 1000;
...
int SubdivEvaluator::increment_accurate(const MeshTopology &mesh,
                                        Matrix3X const &X, int face,
                                        const Vector2 &u, const Vector2 &du,
                                        int *new_face_out, Vector2 *new_u_out,
                                        Vector2 *dir, Scalar *distance,
  ...
  */
  ...
  while (du1_total != 0 && du2_total != 0) {
    ...
    for (int count = 0;; ++count) {
      ...
      // Find the new face	and the coordinates of the crossing point within
      ...
      if (u1_new < 0.f) {
        ...
        const Scalar u2t = u2_old - du2 * dif / du1;
        if ((u2t >= 0.f) && (u2t <= 1.f)) {
          // face_new = adjacency[face].adjface(3); aux = u2t; face_found =
          // true;
          face_new = mesh.face_adj(3, face);
          aux = u2t;
          face_found = true;
      ...
      if ((u2_new > 1.f) && (!face_found)) {
        ...
        const Scalar u1t = u1_old + du1 * dif / du2;
        if ((u1t >= 0.f) && (u1t <= 1.f)) {
          // face_new = adjacency[face].adjface(2); aux = u1t; face_found =
          // true;
          face_new = mesh.face_adj(2, face);
          aux = u1t;
          face_found = true;
...
Matrix3X SubdivEvaluator::evalPointsCustom(const Matrix3X &base) {
  ...
  for (int i = 0; i < localPointCount; i++) {
    ...
    if (maxStencilBase < st.GetSize())
      maxStencilBase = st.GetSize();
  ...
  Far::PatchMap patchmap(*patchTable);
  Matrix3X result(3, controlVertexPtex.size());
  result.setZero();
  ...
  for (int k = 0; k < controlVertexPtex.size(); k++) {
    ...
    Far::PatchTable::PatchHandle const *handle =
        patchmap.FindPatch(sp.face, sp.u[0], sp.u[1]);
    assert(handle);
...
int SubdivEvaluator::minimizeSurfacePoint(const MeshTopology &mesh,
                                          SurfacePoint p, const Matrix3X &X,
  ...
  for (int iter = 0; iter < cap; iter++) {
    std::vector<SurfacePoint> inPoints;
    ...
    int fout;
    Vector2 uout;
    int jumps = increment_u_crossing_edges(mesh, X, x.face, x.u, du, &fout,
                                           &uout, &dir, &distance, &path);
    x.face = fout;
    x.u = uout;
  }
  if (dOld < dNew)
...
Matrix3X SubdivEvaluator::evalPointsCustom(
    const Matrix3X &base, const std::vector<SurfacePoint> &surfacePoints) {
  OpenSubdiv::Far::StencilTable const *localStencilTable =
      getLocalStencilTable();
  int localPointCount = localStencilTable->GetNumStencils();
  ...
  Far::PatchMap patchmap(*patchTable);
  Matrix3X result(3, surfacePoints.size());
  result.setZero();
  ...
  for (int k = 0; k < surfacePoints.size(); k++) {
    ...
    Far::PatchTable::PatchHandle const *handle =
        patchmap.FindPatch(sp.face, u, v);
    assert(handle);
    Far::PatchParam param = patchTable->GetPatchParam(*handle);
    param.Normalize(u, v);
...
Matrix3X SubdivEvaluator::evalPointsPartial(const Matrix3X &localPts) {
  ...
  for (int k = 0; k < controlVertexPtex.size(); k++) {
    ...
    Far::PatchTable::PatchHandle const *handle =
        this->patchMap->FindPatch(sp.face, u, v);
    assert(handle);
    Far::PatchParam param = patchTable->GetPatchParam(*handle);
    param.Normalize(u, v);
...
void SubdivEvaluator::evalPointsPartial(
    const Matrix3X &localPts, const std::vector<SurfacePoint> &surfacePoints,
...
Vector2 SubdivEvaluator::normalize(int face_idx, Vector2 u, int *patch_idx) {
  /*
  //std::cout << patchTable->GetNumPatchesTotal() << ":" <<
patchTable->GetNumPtexFaces() << std::endl; Far::PatchMap patchmap(*patchTable);
Far::PatchTable::PatchHandle const * handle = patchmap.FindPatch(face_idx, u[0],
u[1]); Far::PatchParam param = patchTable->GetPatchParam(*handle); float u1 =
u[0]; float u2 = u[1]; *patch_idx = handle->patchIndex; param.Normalize(u1, u2);
Vector2 result;
result[0] = u1;
result[1] = u2;
assert(u == result);
...
void SubdivEvaluator::evaluateSubdivSurface(
    Matrix3X const &vert_coords, std::vector<SurfacePoint> const &uv,
    Matrix3X *out_S, triplets_t *out_dSdX, triplets_t *out_dSudX,
    triplets_t *out_dSvdX, Matrix3X *out_Su, Matrix3X *out_Sv,
    Matrix3X *out_Suu, Matrix3X *out_Suv, Matrix3X *out_Svv, Matrix3X *out_N,
    Matrix3X *out_Nu, Matrix3X *out_Nv, bool printDebug) const {
  ...
  // assert((uint8_t*) &evaluation_verts_buffer[1] - (uint8_t*)
  // &evaluation_verts_buffer[0] == 12);
  ...
  float pWeights[MAX_NUM_W], dsWeights[MAX_NUM_W], dtWeights[MAX_NUM_W],
      dssWeights[MAX_NUM_W], dttWeights[MAX_NUM_W], dstWeights[MAX_NUM_W];
...
#undef CLEAR
  ...
  Far::LimitStencilTable const *limitTable =
      Far::LimitStencilTableFactory::Create(*refiner, locationArray, cvstencils,
                                            patchTable, opts);

  const std::vector<OpenSubdiv::Vtr::Index> idx_list =
      limitTable->GetControlIndices();
  const std::vector<OpenSubdiv::Vtr::Index> idx_offsets =
      limitTable->GetOffsets();
  ...
  for (size_t i = 0; i < limit_stencil_count; i++) {
    ...
    for (int j = 0; j < control_range; j++) {
      ...
      if (out_S)
        out_S->col(i) += w[j] * coord;
      if (out_Su)
        out_Su->col(i) += wu[j] * coord;
      if (out_Sv)
        out_Sv->col(i) += wv[j] * coord;
      if (out_Suu)
        out_Suu->col(i) += wuu[j] * coord;
      if (out_Svv)
        out_Svv->col(i) += wvv[j] * coord;
      if (out_Suv)
        out_Suv->col(i) += wuv[j] * coord;
      // if (out_dSdX && w[j] != 0) {
      if (out_dSdX) {
