...
Matrix32 principleCurvatureAmbient(Vector3 p, Vector3 dSdu, Vector3 dSdv,
  ...
  principalDirections.col(0) =
      dSdx *
      es.eigenvectors().real().col(0); //* sqrt(es.eigenvalues().real()[0]
  principalDirections.col(1) =
      dSdx *
      es.eigenvectors().real().col(1); // * sqrt(es.eigenvalues().real()[1]);
...
Matrix22 principleCurvatureTangent(Vector3 p, Vector3 dSdu, Vector3 dSdv,
  ...
  Matrix32 pr;
  pr.col(0) = dSdx * es.eigenvectors().real().col(
                         0); //* sqrt(es.eigenvalues().real()[0]
  pr.col(1) = dSdx * es.eigenvectors().real().col(
                         1); // * sqrt(es.eigenvalues().real()[1]);
