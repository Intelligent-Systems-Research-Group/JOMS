...
//
// ST Particles are a simplified parametric-space particle dynamics simulation:
// each particle is assigned a location on the subdivision surface limit that is
...
// particle. An Update() function then applies the velocities to the locations
// and moves the points along the parametric space.
//
...
public:
  ...
  struct FaceInfo {
    ...
    unsigned int adjedges : 8, flags : 8;
    int adjfaces[4];
  };
  ...
  int GetNumParticles() const { return (int)_positions.size(); }

  std::vector<FaceInfo> GetAdjacency() { return _adjacency; }
  ...
  std::vector<Position> &GetPositions() { return _positions; }

  std::vector<float> &GetVelocities() { return _velocities; }
  ...
  static void WarpParticle(std::vector<FaceInfo> const &adjacency, int edge,
                           Position *p, float *dp);
