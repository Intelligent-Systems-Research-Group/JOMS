...
typedef Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic, MyAlignment>
    MatrixX;

// typedef pdouble Modifyable;
// typedef Eigen::Matrix<Modifyable, 3, Eigen::Dynamic, MyAlignment> PMatrix3X;

typedef Eigen::Matrix<size_t, Eigen::Dynamic, Eigen::Dynamic, MyAlignment>
    IMatrixX;
...
template <typename U, typename V> void setConstant(U &u, V &v) {
...
template <typename U, typename V> void assign(U &u, const V &v) {
  assert(u.rows() == v.rows() && u.cols() == v.cols());
  for (size_t i = 0; i < u.rows(); i++) {
    for (size_t j = 0; j < v.cols(); j++) {
      u(i, j) <<= v(i, j);
...
template <typename U, typename V> void extract(U &u, V &v) {
  assert(u.rows() == v.rows() && u.cols() == v.cols());
  for (size_t i = 0; i < u.rows(); i++) {
    for (size_t j = 0; j < v.cols(); j++) {
      v(i, j) >>= u(i, j);
