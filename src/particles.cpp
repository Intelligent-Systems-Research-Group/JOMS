...
public:
  TbbUpdateKernel(float speed, STParticles::Position *positions,
                  float *velocities,
  ...
  void operator()(tbb::blocked_range<int> const &r) const {
    for (int i = r.begin(); i < r.end(); ++i) {
      ...
      assert((p->s > -2.0f) && (p->s < 2.0f) && (p->t > -2.0f) &&
             (p->t < 2.0f));
      ...
      assert((p->s >= 0.0f) && (p->s <= 1.0f) && (p->t >= 0.0f) &&
             (p->t <= 1.0f));
      ...
      if (handle) {
        ...
        _patchCoords[index] = OpenSubdiv::Osd::PatchCoord(*handle, p->s, p->t);
      }
    }
  }
...
STParticles::STParticles(Refiner const &refiner, PatchTable const *patchTable,
  ...
  { // initialize positions
    ...
    for (int i = 0; i < nParticles; ++i) {
      pos->ptexIndex =
          std::min((int)(((float)rand() / (float)RAND_MAX) * nPtexFaces),
                   nPtexFaces - 1);
  ...
  { // initialize topology adjacency
    ...
    int nfaces = refBaseLevel.GetNumFaces(), adjfaces[4], adjedges[4];

    for (int face = 0, ptexface = 0; face < nfaces; ++face) {

      OpenSubdiv::Far::ConstIndexArray fverts =
          refBaseLevel.GetFaceVertices(face);
...
inline void FlipS(STParticles::Position *p, float *dp) {
  p->s = 1.0f - p->s;
  dp[0] = -dp[0];
}

inline void FlipT(STParticles::Position *p, float *dp) {
  p->t = 1.0f - p->t;
  dp[1] = -dp[1];
}

inline void SwapST(STParticles::Position *p, float *dp) {
  std::swap(p->s, p->t);
  std::swap(dp[0], dp[1]);
}

inline void Rotate(int rot, STParticles::Position *p, float *dp) {
...
inline void Trim(STParticles::Position *p) {
  if (p->s < 0.0f)
    p->s = 1.0f + p->s;
  if (p->s >= 1.0f)
    p->s = p->s - 1.0f;
...
inline void Clamp(STParticles::Position *p) {
  if (p->s < 0.0f) {
    p->s = 0.0f;
  } else if (p->s > 1.0f) {
    p->s = 1.0f;
...
inline void Bounce(int edge, STParticles::Position *p, float *dp) {
  switch (edge) {
  case 0:
    assert(p->t <= 0.0f);
    p->t = -p->t;
...
void STParticles::WarpParticle(std::vector<FaceInfo> const &adjacency, int edge,
                               Position *p, float *dp) {
  assert(p->ptexIndex < (int)adjacency.size() && (edge >= 0 && edge < 4));

  FaceInfo const &f = adjacency[p->ptexIndex];

  int afid = f.adjface(edge), aeid = f.adjedge(edge);
...
STParticles::~STParticles() { delete _patchMap; }

void STParticles::Update(float deltaTime) {
  ...
  TbbUpdateKernel kernel(speed, &_positions[0], &_velocities[0], _adjacency,
                         &_patchCoords[0], _patchMap);
  ;
  g_tbbCounter = 0;
  ...
  for (int i = 0; i < GetNumParticles(); ++i, ++p, dp += 2) {
    ...
    OpenSubdiv::Far::PatchTable::PatchHandle const *handle =
        _patchMap->FindPatch(p->ptexIndex, p->s, p->t);
    if (handle) {
      _patchCoords.push_back(OpenSubdiv::Osd::PatchCoord(*handle, p->s, p->t));
    }
  }
#endif
}
...
// Dump adjacency info
std::ostream &operator<<(std::ostream &os, STParticles::FaceInfo const &f) {

  os << "  adjface: " << f.adjfaces[0] << ' ' << f.adjfaces[1] << ' '
     << f.adjfaces[2] << ' ' << f.adjfaces[3] << "  adjedge: " << f.adjedge(0)
     << ' ' << f.adjedge(1) << ' ' << f.adjedge(2) << ' ' << f.adjedge(3)
     << "  flags:";
...
std::ostream &operator<<(std::ostream &os, STParticles const &particles) {
