...
TermBuilder::TermBuilder(std::string _name, int _edgesize,
...
void TermBuilder::checkpoint() { backup_hyperedges = hyperedges; }
...
void TermBuilder::dumpNodes(std::ostream &ostream) {
  ...
  for (int j = 0; j < hyperedges[0].size(); j++) {
    ostream << name << std::setfill('0') << "_" << std::setw(8) << j << ";"
            << name << std::setfill('0') << "_" << std::setw(8) << j << "\n";
    for (int i = 0; i < hyperedges.size(); i++) {
      ostream << labels[i].type << std::setfill('0') << "_" << std::setw(8)
              << hyperedges[i][j] << ";" << labels[i].type << std::setfill('0')
              << "_" << std::setw(8) << hyperedges[i][j] << "\n";
    }
  }
}
...
void TermBuilder::dumpEdges(std::ostream &ostream) {
  ...
  for (int j = 0; j < hyperedges[0].size(); j++) {
    for (int i = 0; i < hyperedges.size(); i++) {
      ostream << labels[i].type << std::setfill('0') << "_" << std::setw(8)
              << hyperedges[i][j] << ";" << name << std::setfill('0') << "_"
              << std::setw(8) << j << ";\"" << name << "\"\n";
    }
  }
}
...
int TermBuilder::get(int edge, int offset) { return hyperedges[edge][offset]; }
...
void TermBuilder::copy() {
  for (int i = 0; i < chyperedges.size(); i++) {
    cudaMemcpy(chyperedges[i], hyperedges[i].data(),
               hyperedges[i].size() * sizeof(int), cudaMemcpyHostToDevice);
  }
}
...
int TermBuilder::count() { return hyperedges[0].size(); }

int TermBuilder::countIdxArrays() { return hyperedges.size(); }
