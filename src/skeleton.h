...
namespace hyp {
	...
	private:
		...
		void set_joint_location(std::string from, std::string to, Vector3 position, std::vector<Vector3>& joint_positions);
	public:
	...
	void add(std::map<std::string, std::vector<size_t>>& vertex_groups,std::vector<bool>* output);
		...
		std::vector<Scalar> extractVertexWeights(const Weights& weights, size_t index);
        SparseMatrix extractWeightMatrix(const Weights& weights, size_t n);
		void setPose(std::vector<Matrix3>& vec_theta, std::map<std::pair<std::string, std::string>,
			Vector3>& joint_locations, std::vector<Matrix4>& absolutes);
		Vector3 skin_vertex(std::vector<Matrix4>& absolutes, const VectorX& weights, const Vector3& position);
