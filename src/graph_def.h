...
private:
  ...
  void implicit_update(int iteration, int failCount, MeshTopology *top,
                       bool fake);

  void continuous_update(int iteration, int failCount, MeshTopology *top,
                         bool fake);
  void renormalize();
...
public:
  ...
  void run();
  void dump(const Model &model, const MeshTopology &top, bool real, int iter,
            int pcs, int qcs);
