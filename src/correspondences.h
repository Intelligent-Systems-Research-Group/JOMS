...
class ISurfaceState {
public:
  virtual int move(int stateId, Vector2 u, int f, Vector2 d, Vector2 &out,
                   int &fout, Scalar *dist,
                   std::vector<Vector3> *path = NULL) = 0;
  virtual Vector3 moveTangent(int stateId, Vector2 u, int f, Vector2 d) = 0;
  virtual void alternatingUpdate(int stateId, int dataIndex, Vector2 u, int f,
                                 Vector2 d, Vector2 &out, int &fout) = 0;
  virtual bool discreteUpdate(int stateId, int dataIndex, Vector2 &out,
                              int &fout) = 0;
...
class SurfaceState : public ISurfaceState {
  ...
  public:
    MeshTopology *mesh;
    SingleState(int index, SubdivEvaluator *eval,
                std::shared_ptr<RayTracer> _rayTracer, MeshTopology *mesh,
                Matrix3X *scan, Matrix3X *scan_normals);
    virtual int move(Vector2 u, int f, Vector2 d, Vector2 &out, int &fout,
                     Scalar *distance, std::vector<Vector3> *path = NULL);
    ...
    virtual void setCurrentModel(Matrix3X &pts, Matrix3X &localPts,
                                 const std::vector<int> &excludedVertices,
                                 bool rebuild_tree);
    virtual void alternatingUpdate(int dataIndex, Vector2 u, int f, Vector2 d,
...
public:
  SurfaceState();
  void addData(SubdivEvaluator *eval, std::shared_ptr<RayTracer> _rayTracer,
               MeshTopology *mesh, Matrix3X *scan, Matrix3X *scan_normals);
  virtual int move(int stateId, Vector2 u, int f, Vector2 d, Vector2 &out,
  ...
  virtual void setCurrentModel(int stateId, Matrix3X &pts, Matrix3X &localPts,
                               ...
                               bool rebuild_tree);
  virtual void alternatingUpdate(int stateId, int dataIndex, Vector2 u, int f,
                                 Vector2 d, Vector2 &out, int &fout);
  virtual bool discreteUpdate(int stateId, int dataIndex, Vector2 &out,
                              int &fout);
