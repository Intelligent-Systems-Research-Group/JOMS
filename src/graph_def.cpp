...
void dumpCloudA(std::string name, int iter, const Matrix3X &cloud,
...
Problem::Problem(const InputParams &ip, const Corpora &_corpora)
    : inputParams(ip), vmarkers(ip.nscans), vreal(ip.nscans), cloud(ip.nscans),
      model2Data(ip.nscans), cloudNorm(ip.nscans), topologies(ip.nscans),
      corpora(_corpora), skeleton(ip.template_skeleton_path),
      REST_SIZE((1 + ip.ncomp + (ip.ncomp + 1) +
                 ip.ndeformjoints * (ip.ndeformshapes + 2 + 1) + 1)),
      MODEL_SIZE(23), PJOINT_SIZE(1 + 1 + 2 * ip.ncomp), SCALE(1.0f),
      LAMBDA(100)

{
  ...
  std::cout << "collectPerKernelTimingInfo " << param.collectPerKernelTimingInfo
            << std::endl;
  std::cout << "threadsPerBlock " << param.threadsPerBlock << std::endl;

  std::cout << "Start NewState" << std::endl;
  state = Opt_NewState(param);
  problem = Opt_ProblemDefine(state, ip.optscript_path.c_str(),
...
void Problem::createModelWeights(const Model &initModel, int isFree) {
  ...
  barriers.joints =
      barriers.shape + (inputParams.ncomp + 1) * inputParams.nverts;
  ...
  barriers.deform =
      barriers.joints + (inputParams.ncomp + 1) * inputParams.njoints;

  int defn = inputParams.ndeformjoints * inputParams.ndeformshapes;
  ...
  barriers.poseMean = barriers.deform + inputParams.nverts *
                                            inputParams.ndeformshapes *
                                            inputParams.ndeformjoints;
  ...
  if (freeVars) {
    barriers.end1d =
        barriers.weights + inputParams.njoints * inputParams.nverts;
  } else {
    barriers.endD1 =
        barriers.weights + inputParams.njoints * inputParams.nverts;
  }
...
bool Problem::loadData(MeshTopology *top) {
  ...
  Model initModel;
  assert(ip.ndeformjoints * ip.ndeformshapes == 18 ||
         ip.ndeformjoints * ip.ndeformshapes == 0);
  initModel.init(control_vertices, myJointPrior, skinningWeights, ip.ncomp,
                 ip.ndeformjoints * ip.ndeformshapes, ip.npersons, ip.nscans);
  ...
  for (int i = 0; i < m; i++) {
    ...
    std::string scanpath = scanbase +
                           corpora.folder[corpora.scan_to_person_id[i]] + "/" +
    ...
    std::cout << i << " try start loading " << scanpath
              << " flipped: " << corpora.flipped[i] << std::endl;
    ...
    std::cout << "normals: " << normals.cols() << "\t Masks" << scanMask.size()
              << std::endl;
    ...
    std::cout << "Scanstats " << points.cols() << " " << normals.cols()
              << std::endl;
    ...
    {
      vmarkers[i].setZero();
      for (auto it = marker_vertex_groups.begin();
           it != marker_vertex_groups.end(); ++it) {
    ...
    */
    ...
    // greedyFarthestSampling(points, normals, ip.npoints, &cloud[i],
    // &cloudNorm[i]);

    // saveObj(std::string("out/gtmeth")+ std::to_string(i)
    // +std::string(".obj"),  top,  &v[i]);
    std::cout << "load success " << corpora.rot[i].transpose() << std::endl;
  }

  OpenSubdiv::Far::StencilTable const *localStencilTable =
      eval->getLocalStencilTable();
  ...
  for (int i = 0; i < m; i++) {
    ...
    Matrix3X subdivCloud;
    subdivCloud.resize(3, ip.npoints);
    ...
    dumpCloudA(folder + std::string("labels") + std::to_string(i), i,
               vmarkers[i]);

    std::cout << "scan: " << i << std::endl;
    dumpCloudA(folder + std::string("cloud") + std::to_string(i), i, cloud[i],
               &cloudNorm[i]);
  ...
  assert(marker_vertex_groups.size() == ip.nlabels);
  for (auto markerIt = marker_vertex_groups.begin();
       markerIt != marker_vertex_groups.end(); ++markerIt) {
    double u, v;
    int fidx, pidx;
    Vector2 pt;
  ...
  barriers.labelSurface2d =
      barriers.label2d + ip.nviews * ip.nscans * ip.njointlabels;
  for (int v = 0; v < ip.nviews; v++) {
    for (int i = 0; i < m; i++) {
      std::string pose2d_path = pose2dbase +
                                corpora.folder[corpora.scan_to_person_id[i]] +
                                "/" + corpora.scan_names[i] + std::string("_") +
                                std::to_string(v) + "_keypoints.json";
      Pose2d pose2d(ip.template_surface_map_path);
  ...
  barriers.labelSurfaceWeights2d =
      barriers.labelWeights2d + ip.nviews * ip.njointlabels * ip.nscans;
  for (int v = 0; v < ip.nviews; v++) {
    for (int i = 0; i < m; i++) {
      std::string pose2d_path = pose2dbase +
                                corpora.folder[corpora.scan_to_person_id[i]] +
  ...
  barriers.metric =
      barriers.labelSurfaceWeights2d + ip.nviews * ip.nverts * ip.nscans;
  // std::cout << "Check boundary sanity " << barriers.metric << " : " <<
  // bsplines_idx.size() << std::endl;
  ...
  for (int i = 0; i < 2 * inputParams.nscans; i++) {
    ...
    for (int j = 0; j < inputParams.njoints; j++) { //+trans
      ...
      // rot.setZero();
      assert(initModel.scans[i % inputParams.nscans].posebase.cols() ==
             inputParams.njoints);
  ...
  barriers.meanShapePrior =
      barriers.cameraViews + 2 * ip.nviews * ip.camPaths.size();
  for (int i = 0; i < inputParams.nverts; i++) {
    Scalar3 pt;
  ...
  saveObj(ip.out_path + std::string("/meanShapePrior.obj"), top,
          &initModel.mean);

  std::cout << "End Data Insertion: "
            << (barriers.meanShapePrior + inputParams.nverts) << std::endl;
  ...
  barriers.endD1 =
      barriers.weightInit + inputParams.nverts * inputParams.njoints;
  ...
  inputData.verifyDataSize(barriers.endD1, barriers.endD2, barriers.endD3,
                           barriers.endDW);
  return true;
...
void Problem::createUnknowns(MeshTopology *top) {
  ...
  barriers.end2d =
      barriers.surfaceUnknown + inputParams.nscans * inputParams.npoints;
  ...
  for (int i = 0; i < inputParams.nscans; i++) {
    ...
    // saveObj(std::string("out/Ta")+ std::to_string(iter) + std::string("_") +
    // std::to_string(i)+std::string(".obj"), &top, &rotated);
  ...
  barriers.localRegistration =
      barriers.registration + inputParams.nscans * inputParams.nverts;
  ...
  for (int k = 0; k < inputParams.nscans; k++) {
    ...
    Matrix3X T;
    T.resize(3, inputParams.nverts);
    inputData.read(T, barriers.registration + k * inputParams.nverts,
                   inputParams.nverts);
    ...
    if (corpora.person_fist[current_person])
      surfaceState.setCurrentModel(k, T, localPts, excludedVertices,
                                   true); // false
    else
      surfaceState.setCurrentModel(k, T, localPts, noExcludedVertices, true);

    // saveObj(std::string("out/gtmeth")+ std::to_string(k)
    // +std::string(".obj"),  top,  &T); dumpCloudA(std::string("out/p"), k,
    // localPts); Matrix3X direct = eval->localPoints(T);
  ...
  barriers.debugRegistration =
      barriers.localRegistration + inputParams.nscans * localPointCount;
  ...
  barriers.pose =
      barriers.debugRegistration + inputParams.nlabels * inputParams.nscans;
  ...
  barriers.absolutePose =
      barriers.pose + (inputParams.njoints + 1) * inputParams.nscans;

  barriers.pjoint =
      barriers.absolutePose + (inputParams.njoints + 1) * inputParams.nscans;
  ...
  barriers.sjoint =
      barriers.pjoint + inputParams.njoints * inputParams.npersons;
  ...
  // barriers.end3d = barriers.pjoint +
  // (inputParams.njoints)*inputParams.npersons; //inputParams.njoints+1
  // std::cout << barriers.end3d << ":" << std::endl;

  inputData.verifySize(barriers.end3d, barriers.end2d, barriers.end1d);
}
...
void Problem::createReverseDataEdges(MeshTopology *top, int tid) {
  ...
  for (int i = 0; i < ip.nscans; i++) {
    for (int v = 0; v < ip.nverts; v++) {
      ...
      std::cout << top->num_faces() << "," << i << "," << v << "," << fidx
                << "," << pidx << std::endl;
      ...
      for (int q = 0; q < 16; q++) {
        ...
        edge[offset3d++] =
            barriers.localRegistration + i * localPointCount + offset;
      }
      terms[tid].add(edge);
    }
  }
...
void Problem::createLabelTerm(MeshTopology *top) {
  ...
  // int size = corpora.markerFrames < ip.nscans ? corpora.markerFrames :
  // ip.nscans;
  for (int i = 0; i < ip.nscans; i++) {
    int current_person = corpora.scan_to_person_id[i];
    if (!corpora.person_has_labels_3d[current_person])
  ...
  for (int v = 0; v < ip.nviews; v++) {
    for (int i = 0; i < ip.nscans; i++) {
      ...
      for (int j = 0; j < ip.njointlabels; j++) {
        ...
        edge[offset3d++] = barriers.label2d + v * ip.nscans * ip.njointlabels +
                           (ip.njointlabels * i) + j;
        edge[offset3d++] = barriers.labelWeights2d +
                           v * ip.nscans * ip.njointlabels +
                           (ip.njointlabels * i) + j;
        ...
        edge[offset3d++] =
            barriers.cameraViews + 2 * ip.nviews * camId + 2 * v + 1;
        terms[tid].add(edge);
      }
      ...
      for (int j = 0; j < ip.nverts; j++) {
        int offset3d = 0;
        int camId = corpora.person_fist[current_person] ? 1 : 0;
        edge[offset3d++] = barriers.labelSurface2d + v * ip.nscans * ip.nverts +
                           (ip.nverts * i) + j;
        edge[offset3d++] = barriers.labelSurfaceWeights2d +
  ...
  for (int i = 0; i < ip.nscans; i++) {
    ...
    for (int j = 0; j < ip.nverts; j++) {
      ...
      for (int k = 0; k < ip.ndeformjoints; k++) {
        ...
        edge[offset3d++] =
            !skeleton.isRigid(joint, fist) ? barriers.one : barriers.zero;
      }
      std::cout << std::endl;
      for (int k = 0; k < ip.ncomp; k++) {
      ...
      edge[offset3d++] =
          i < corpora.markerFrames ? barriers.one : barriers.zero;
      // std::cout << "Size Rest Model Debug: " << offset3d << "/" <<
      // edge.size() << std::endl;
  ...
  for (int i = 0; i < ip.nscans; i++) {
    ...
    for (int j = 0; j < ip.nverts; j++) {
      ...
      for (int joint = 0; joint < ip.njoints; joint++) {
        if (jointInInfluence(j, joint)) {
          edge[offset3d++] =
              barriers.absolutePose + (ip.njoints + 1) * i + joint + 1;
        }
      }
      for (int joint = 0; joint < ip.njoints; joint++) {
        if (jointInInfluence(j, joint)) {
          edge[offset3d++] =
              barriers.absolutePoseBase + (ip.njoints) * i + joint;
        }
      }
      assert(edge.size() == edgeSize && edgeSize == offset3d);
      terms[tid].add(edge);
      ...
      for (auto joint : joints) {
        for (int m = 0; m < 4; m++) {
          ...
          for (int j = 0; j < 4; j++) {
            ...
            if (kk == -1) { /*kk == -1*/
              edge[0 + j] =
                  barriers.deform + (9 * 0 + k) * ip.nverts + u; // fake
              edge[4 + j] = barriers.zero;
              edge[8 + j] = barriers.metric + 16 * i + 4 * m + j;
  ...
  for (int i = 0; i < ip.npersons; i++) {
    for (int k = 0; k < ip.ncomp; k++) {
      ...
      std::cout << "Person Count: "
                << inputData.readPoint1d(barriers.personCount + i) << std::endl;
      terms[tid].add(edge);
    }
  }
}
...
void Problem::createGroundRepelTerm(MeshTopology *top) {
  ...
  for (int i = 0; i < ip.nscans; i++) {
    ...
    for (int v = 0; v < ip.nverts; v++) {
      ...
      edge[offset3d++] = barriers.vertexLabel + v;
      edge[offset3d++] =
          barriers.singleZero + corpora.person_ground_axes[person];
...
void Problem::createJointReg() {
  ...
  for (size_t i = 0; i < ip.njoints - 1; i++) {
    ...
    auto vertex_ring = border_groups.at(secondKey);
    // std::cout << secondKey.first << ":" << secondKey.second << ":" <<
    // vertex_ring.size() <<std::endl; auto lu = jointMap.at(secondKey);
  ...
  for (int i = 0; i < ip.nscans; i++) {
    for (int j = 0; j < localPointCount; j++) {
      ...
      for (int k = 0; k < LOCAL_SIZE; k++) {
        assert(localIdx[j * LOCAL_SIZE + k] >= 0 &&
               localIdx[j * LOCAL_SIZE + k] < ip.nverts);
        edge[offset3d++] = barriers.registration + i * ip.nverts +
                           localIdx[j * LOCAL_SIZE + k];
      }
      terms[tid].add(edge);
    }
  }
...
void Problem::createSymmetryTerm() {
  ...
  for (int c = 0; c < ip.ncomp + 1; c++) {
    ...
    for (int i = 0; i < ip.njoints; i++) {
      ...
      std::cout << "(" << jI.first << "," << jI.second << ") <-> (" << jJ.first
                << "," << jJ.second << ")" << std::endl;
      terms[tid].add(edge);
    }
  }
...
void Problem::initOpt() {
  ...
  Opt_SetSolverParameter(state, plan, "nIterations",
                         (void *)&nNonLinearIterations);
  Opt_SetSolverParameter(state, plan, "lIterations",
                         (void *)&nLinearIterations);
...
void Problem::createOptParams() {
  ...
  for (int i = 0; i < terms.size(); i++) {
    ...
    std::cout << "Term " << i << " with " << (terms[i].countIdxArrays() + 1)
              << " params" << std::endl;
  }
...
void Problem::retrieve(Model &model) {
  ...
  for (int j = 0; j < inputParams.njoints - 1; j++) {
    Scalar3 meanBase = inputData.readPoint3d(barriers.poseBaseMean + j);
    Matrix3X X;
    X.resize(3, 1);
  ...
  for (int i = 0; i < ip.ncomp; i++) {
    ...
    inputData.readDynamic(model.pcs[i], barriers.shape + (1 + i) * ip.nverts,
                          ip.nverts);
  }
  ...
  for (int i = 0; i < ip.npersons; i++) {
    ...
    inputData.read(person.joints, barriers.pjoint + i * inputParams.njoints,
                   inputParams.njoints);
  ...
  for (int i = 0; i < ip.ncomp + 1; i++) {
    ...
    if (i == 0) {
      ...
      inputData.readDynamic(model.jointpcs[i - 1],
                            barriers.joints + i * inputParams.njoints,
                            inputParams.njoints);
    }
  }
  ...
  for (int i = 0; i < inputParams.nscans; i++) {
    ...
    inputData.read(scan.trans, barriers.pose + (inputParams.njoints + 1) * i,
                   1);
    inputData.read(scan.absoluteTrans,
                   barriers.absolutePose + (inputParams.njoints + 1) * i, 1);
    inputData.read(scan.joints, barriers.sjoint + i * inputParams.njoints,
                   inputParams.njoints);

    scan.T.resize(3, inputParams.nverts);
    inputData.read(scan.T, barriers.registration + (inputParams.nverts) * i,
                   inputParams.nverts);
    ...
    for (int v = 0; v < ip.nviews; v++) {
      ...
      {
        scan.label2dRays[v].col(ip.njointlabels + ip.nverts) =
    ...
    for (int j = 0; j < inputParams.njoints; j++) {
      Scalar3 thetabase = inputData.readPoint3d(barriers.poseBase +
                                                i * inputParams.njoints + j);
      ...
      Matrix3X theta;
      theta.resize(3, 1);
      inputData.read(theta,
                     barriers.pose + i * (inputParams.njoints + 1) + 1 + j, 1);
      scan.pose(0, j) = theta(0, 0);
      scan.pose(1, j) = theta(1, 0);
      scan.pose(2, j) = theta(2, 0);
    }
    for (int j = 0; j < inputParams.njoints; j++) {
      Scalar3 thetabase = inputData.readPoint3d(barriers.absolutePoseBase +
                                                i * inputParams.njoints + j);
      ...
      theta.resize(3, 1);
      inputData.read(
          theta, barriers.absolutePose + i * (inputParams.njoints + 1) + 1 + j,
          1);
    ...
    inputData.read(scan.Trest, barriers.pshape + (inputParams.nverts) * i,
                   inputParams.nverts);
    ...
    inputData.read(scan.Tlocal,
                   barriers.localRegistration + localPointCount * i,
                   localPointCount);
    ...
    Matrix3X Xdu;
    Xdu.resize(3, ip.npoints);
    Matrix3X Xdv;
    Xdv.resize(3, ip.npoints);

    // eval->evaluateSubdivSurface(scan.T,
    //    surfacePoints, &scan.points,NULL,NULL,NULL,
    //	&Xdu, &Xdv);
    eval->evalPointsPartial(scan.Tlocal, surfacePoints, &scan.patchPoints, &Xdu,
                            &Xdv);
    Matrix3X tangentPoint;
    tangentPoint.resize(3, ip.npoints);

    Matrix2X deltaU;
    deltaU.resize(2, ip.npoints);
    inputData.read(deltaU, barriers.surfaceUnknown + i * ip.npoints,
                   ip.npoints);
    ...
    for (int p = 0; p < ip.npoints; p++) {
      ...
      {
        Scalar2 uS =
            inputData.readPoint2d(barriers.surfaceScale + i * ip.npoints + p);
        us[0] = uS.x;
        us[1] = uS.y;
      }

      tangentPoint.col(p) =
          scan.patchPoints.col(p) +
    ...
    scan.labels.resize(3, ip.nlabels);
    scan.labels.setZero();
    ...
    eval->evaluateSubdivSurface(scan.T, labelPoints, &scan.labels);

    scan.labels_initial.resize(3, ip.nlabels);
    inputData.read(scan.labels_initial,
                   barriers.debugRegistration + ip.nlabels * i, ip.nlabels);
    ...
    eval->evalPointsPartial(scan.Tlocal, labelPoints, &scan.labels_partial,
                            NULL, NULL);
  ...
  static int bla = 0;
  model.weights.resize(ip.njoints, ip.nverts);
  model.weights.setZero();
  for (int i = 0; i < ip.nverts; i++) {
    SVector4 influence = closestJoints.col(i).cast<int>();
    ...
    for (int j = 0; j < 4; j++) {
      int joint = influence[j];
      Scalar w = inputData.readDynamic1d(barriers.weights +
                                         inputParams.njoints * i + joint) /
                 LAMBDA;
    ...
    for (int j = 0; j < 4; j++) {
      ...
      Scalar w = inputData.readDynamic1d(barriers.weights +
                                         inputParams.njoints * i + joint) /
                 LAMBDA;
      // Scalar wp = inputData.read(barriers.weights+inputParams.njoints*i +
      // joint) / LAMBDA; model.weights(j,i) =
      // model.weights(j,i)*model.weights(j,i); model.weights(joint,i) = 1.0 / (1
      // + exp(-LAMBDA*w));
  ...
  MatrixX raw;
  raw.resize(ip.njoints, ip.nverts);
  for (int i = 0; i < ip.nverts; i++) {
    for (int j = 0; j < ip.njoints; j++) {
      raw(j, i) = inputData.readDynamic1d(barriers.weights +
                                          inputParams.njoints * i + j);
    }
  }
  if (bla % 50 == 0) {
  ...
  for (int i = 0; i < ip.ndeformjoints * ip.ndeformshapes; i++) {
    ...
    inputData.readDynamic(model.deform[i], barriers.deform + ip.nverts * i,
                          ip.nverts);
  }
  ...
  for (int i = 0; i < ip.nscans; i++) {
    ...
    skeleton.calculate_kinematic_chain(skinjoints, localrot, absolutes);

    for (int v = 0; v < ip.nverts; v++) {
      VectorX wv = model.weights.col(v);
      wv.setZero();
...
void Problem::removeRigidJoints() {
  ...
  std::vector<std::string> jointsToRemove = {
      "m_neck", "m_head", "m_rlowerarm", "m_rhand",
      /*
                      "m_rthumb",
                      "m_rthumb-proximal",
                      "m_rthumb-distal",
      ...
      */
      ...
      /*
                     ...
                     "m_lpinky-middle",
      */
      "m_hip", "m_pelvis", "m_rupperleg", "m_rlowerleg", "m_rfoot",
      "m_lupperleg", "m_llowerleg", "m_lfoot"

  };
  int restJointColBase3 =
      2 + ip.ncomp + ip.ndeformjoints * ip.ndeformshapes + 2;
  for (int i = 0; i < ip.nscans; i++) {
    for (int j = 0; j < ip.nverts; j++) {
      for (int k = 0; k < ip.ndeformjoints; k++) {
        ...
        std::vector<std::string>::iterator it =
            std::find(jointsToRemove.begin(), jointsToRemove.end(), joint);
        if (it == jointsToRemove.end())
          continue;
        // assert(jointIdx == *it);

        terms[modelTermIndex].set(restJointCol3, restJointRow3, barriers.one);
      }
    }
    for (int k = 0; k < ip.njoints; k++) {
...
void Problem::icp_update(int iteration, MeshTopology *top, bool alternating) {
  ...
  std::vector<SurfacePoint> surfacePoints;
  surfacePoints.resize(ip.npoints);
  ...
  Matrix3X surface3d;
  surface3d.resize(3, ip.npoints);
  Matrix3X dSdu;
  dSdu.resize(3, ip.npoints);
  Matrix3X dSdv;
  dSdv.resize(3, ip.npoints);
  Matrix3X dSduu;
  dSduu.resize(3, ip.npoints);
  Matrix3X dSduv;
  dSduv.resize(3, ip.npoints);
  Matrix3X dSdvv;
  dSdvv.resize(3, ip.npoints);

  Matrix3X c1;
  c1.resize(3, ip.npoints);
  Matrix3X c2;
  c2.resize(3, ip.npoints);
  ...
  for (int i = 0; i < ip.nscans; i++) {
    ...
    inputData.read(localPts, barriers.localRegistration + i * localPointCount,
                   localPointCount);
    // surfaceState.setCurrentModel(i,T, localPts, true);
    int current_person = corpora.scan_to_person_id[i];
    if (corpora.person_fist[current_person] &&
        iteration < ip.vertex_masking_end)
      surfaceState.setCurrentModel(i, T, localPts, excludedVertices,
                                   true); // false
    else
    ...
    for (int p = 0; p < ip.npoints; p++) {
      ...
      {
        Scalar2 ur =
            inputData.readPoint2d(barriers.surface + i * ip.npoints + p);
        uold[0] = ur.x; // = 0;
        uold[1] = ur.y; // = 0;
      }
...
void Problem::continuous_update(int iteration, int failCount, MeshTopology *top,
                                bool fake) {
  InputParams &ip = inputParams;
  inputData.retrieve();
  double error = 0;
  ...
  for (int i = 0; i < ip.nscans; i++) {
    ...
    for (int p = 0; p < ip.npoints; p++) {
      inputData.resetUnknown2d(barriers.surfaceUnknown + i * ip.npoints + p);
      Vector2 u;
      {
        Scalar2 ur =
            inputData.readPoint2d(barriers.surface + i * ip.npoints + p);
      ...
      step[1] *= us[1];
      int jumpCount =
          surfaceState.move(i, u, f, step, unew, fnew, &singleDistance);
      if (f != fnew)
        faceJump++;
      f = fnew;
      u = unew;
...
mkdir(folder.c_str(),0777);
folder = folder + "/";
    std::string path = folder + std::string("cont_")+ std::to_string(iteration)
+ std::string("_") + std::to_string(i)+std::string("fail") +
std::to_string(failCount) + std::string(".obj"); quiver(path,X,Y);
...
void Problem::implicit_update(int iteration, int failCount, MeshTopology *top,
                              bool fake) {
  InputParams &ip = inputParams;
  inputData.retrieve();
  double error = 0;
  ...
  for (int i = 0; i < ip.nscans; i++) {
    ...
    for (int p = 0; p < ip.npoints; p++) {
      ...
      int jumpCount = surfaceState.move(i, u, f, deltaU.col(p), unew, fnew,
                                        &singleDistance);
      if (f != fnew)
        faceJump++;
      f = fnew;
...
void Problem::renormalize() {
  ...
  for (int v = 0; v < ip.nverts; v++) {
    ...
    for(int infu =   0; infu < 4; infu++) {
        int joint = influence[infu];
        float w = inputData.read(barriers.weights+inputParams.njoints*v +
    joint); maximum = fmax(maximum,w);
    }
    for(int infu =   0; infu < 4; infu++) {
        int joint = influence[infu];
    ...
    */
    ...
    for (int infu = 0; infu < 4; infu++) {
      int joint = influence[infu];
      Scalar w =
          inputData.read(barriers.weights + inputParams.njoints * v + joint);
      sum += exp(w);
    }
    for (int infu = 0; infu < 4; infu++) {
      int joint = influence[infu];
...
void Problem::propagateModel(MeshTopology *top) {
  ...
  for (int k = 0; k < inputParams.nscans; k++) {
    ...
    for (int i = 0; i < localPointCount; i++) {
      ...
      BarrierIndex index = barriers.localRegistration + k * localPointCount + i;
...
void Problem::dumpModel(int i, MeshTopology *top) {
  ...
  for (int p = 0; p < ip.npersons; p++) {
    ...
    // std::cout << dumppath << " at " << scanStart << ":" << scanEnd <<
    // std::endl;
...
void Problem::testUpdate(MeshTopology *top) {
  InputParams &ip = inputParams;
  Matrix3X localPts(3, localPointCount);
  inputData.read(localPts, barriers.localRegistration + 0 * localPointCount,
                 localPointCount);
...
void Problem::run() {
  ...
  loadVertexGroups(ip.template_vgroups_path, &vertex_groups, &border_groups,
                   &weights); // vgroups.txt _fixmid.txt
...
#ifdef USE_MARKERS
  loadVertexGroups(ip.template_marker_path, &marker_vertex_groups, NULL,
                   NULL); // markers.txt.old
#endif
...

  {
    ...
    for (auto i : border_groups) {
      ...
      std::cout << j << std::endl;
      borderIndices[j].insert(borderIndices[j].end(), i.second.begin(),
                              i.second.end());
    }

    Matrix4X jointDistance;
    closestJoints = top.findClosest(control_vertices, borderIndices,
                                    sparseWeights, &jointDistance);
  }
  {
    std::vector<std::vector<int>> borderIndices(border_groups.size());
    for (auto i : border_groups) {
      if (i.first.first == "m_root" || i.first.second == "m_root")
        continue;
      int j = skeleton.getJointIndex(i.first.first, i.first.second);
      borderIndices[j].insert(borderIndices[j].end(), i.second.begin(),
                              i.second.end());
    ...
    Matrix4X jointDistance;
    closestDeformJoints = top.findClosest(control_vertices, borderIndices,
                                          sparseWeights, &jointDistance);
  }
  for (auto excluded_part : ip.exclude_icp_vertex_groups) {
    for (size_t v : vertex_groups[excluded_part]) {
      if (std::find(excludedVertices.begin(), excludedVertices.end(), v) ==
  ...
  if (optimize) {
    ...
    for (int i = 0; i < inputParams.iteration_end; i++) { // 150
      ...
      while (true) {
        ...
        double afterManifold =
            Opt_ProblemCurrentCost(state, plan, &optinput[0]);
        ...
        std::cerr << i << " : " << cost << " -> " << euclidCost << " -> "
                  << afterManifold << " -> " << afterContinuous << " -> "
                  << newcost << std::endl;
        std::cerr << "Data " << i << " : " << dataBefore << " -> " << dataAfter
...
void Problem::dump(const Model &model, const MeshTopology &top, bool real,
                   int iter, int pc_limit, int qc_limit) {
  InputParams &ip = inputParams;
  ...
  saveObj(ip.out_path + std::string("/c") + std::to_string(iter) +
              std::string("mean.obj"),
          &top, &mymean);
  dumpCloud(ip.out_path + std::string("/cJ_") + std::string("mean"), iter,
            mymeanjoint);
  for (int j = 0; j < pc_limit; j++) {
    Matrix3X pc = model.pcs[j];
    Matrix3X temp = mymean + 3 * pc;
    saveObj(ip.out_path + std::string("/c") + std::to_string(iter) +
                std::string("pc") + std::to_string(j) + std::string(".obj"),
            &top, &temp);
    const Matrix3X &jpc = model.jointpcs[j];
    Matrix3X jtemp = mymeanjoint + 3 * jpc;
    dumpCloud(ip.out_path + std::string("/cJ") + std::string("pc") +
                  std::to_string(j) + "_",
              iter, jtemp);
  }
  ...
  for (int i = 0; i < ip.nscans; i++) {
...
#ifdef FULL_DUMP
    dumpCloudA(folder + std::string("jrest") + std::to_string(iter) +
                   std::string("_") + std::to_string(i),
               i, J1);

    saveObj(folder + std::string("Prest") + std::to_string(iter) +
                std::string("_") + std::to_string(i) + std::string(".obj"),
            &top, &C1);
#endif
    std::vector<Vector3> skinjoints;
    std::vector<Matrix3> localrot;
    ...
    skeleton.calculate_kinematic_chain(skinjoints, absrot, unlinked,
                                       model.scans[i].joints);
    ...
    Matrix3X jointsOut;
    jointsOut.resize(3, absolutes.size());
...
#ifdef FULL_DUMP
    dumpCloudA(folder + std::string("pj") + std::to_string(iter) +
                   std::string("_") + std::to_string(i),
               i, person.joints);
    dumpCloudA(folder + std::string("j") + std::to_string(iter) +
                   std::string("_") + std::to_string(i),
               i, jointsOut);
    // dumpCloudA(folder + std::string("sj")+ std::to_string(iter) +
    // std::string("_") + std::to_string(i), i, jointsOut);
    if (corpora.person_has_labels_2d[personId]) {
      for (int v = 0; v < ip.nviews; v++) {
        ...
        Matrix3X footpoints;
        footpoints.resize(3, ip.njointlabels);
        for (int k = 0; k < ip.njointlabels; k++) {
          int camId = corpora.person_fist[personId] ? 1 : 0;
          std::string camPath = ip.camPaths[camId];

          Camera cam(camPath, v);
          footpoints.col(k) = cam.projectJointOnRay(
              model.scans[i].label2dRays[v].col(k), sj.col(k));
        }

        Matrix3X sjn = -footpoints + sj;

        dumpCloudA(folder + std::string("latent joint") + std::to_string(iter) +
                       std::string("_") + std::to_string(v) + "_",
                   i, sj, &sjn);
    ...
    if (ip.useModel2Data) {
      Matrix3X m2d;
      m2d.resize(3, ip.nverts);
      for (int k = 0; k < ip.nverts; k++) {
        m2d.col(k) = cloud[i].col(model2Data[i][k]);
      }
      Matrix3X m2dn = model.scans[i].T - m2d;
      dumpCloudA(folder + std::string("m2d") + std::to_string(iter) +
                     std::string("_") + std::to_string(i),
                 i, m2d, &m2dn);
    }
    saveObj(folder + std::string("c") + std::to_string(iter) +
                std::string("_pc") + std::to_string(pc_limit) +
                std::string("_") + std::string("_qc") +
                std::to_string(qc_limit) + std::string("_") +
                std::to_string(i) + std::string(".obj"),
            &top, &skinned);
    // dumpSubivA(folder + std::string("csub")+ std::to_string(iter) +
    // std::string("_") + std::to_string(i)+std::string(".obj"),
    //    iter, eval, skinned);
    saveObj(folder + std::string("u") + std::to_string(iter) +
                std::string("_") + std::to_string(i) + std::string(".obj"),
            &top, &unlinked_mesh);

    saveObj(folder + std::string("T") + std::to_string(iter) +
                std::string("_") + std::to_string(i) + std::string(".obj"),
            &top, &model.scans[i].T);
    // dumpSubivA(folder + std::string("Tsub")+ std::to_string(iter) +
    // std::string("_") + std::to_string(i)+std::string(".obj"), 	iter, eval,
    //model.scans[i].T);
#ifdef FULL_DUMP
    saveObj(folder + std::string("Trest") + std::to_string(iter) +
                std::string("_") + std::to_string(i) + std::string(".obj"),
            &top, &model.scans[i].Trest);
#endif
    Matrix3X normals = cloud[i] - model.scans[i].patchPoints;
    dumpCloudA(folder + std::string("S") + std::to_string(iter) +
                   std::string("_") + std::to_string(i),
               i, model.scans[i].patchPoints, &normals);
    Matrix3X normLabel = vmarkers[i] - model.scans[i].labels;

    MeshTopology patchMesh;
    Matrix3X patchVertices;
    eval->createPatches(model.scans[i].Tlocal, &patchMesh, &patchVertices);
    saveObj(folder + std::string("Patch") + std::to_string(iter) +
                std::string("_") + std::to_string(i) + std::string(".obj"),
            &patchMesh, &patchVertices);

    Matrix3X tangentNormals = cloud[i] - model.scans[i].tangentStep;
    dumpCloudA(folder + std::string("TanS") + std::to_string(iter) +
                   std::string("_") + std::to_string(i),
               i, model.scans[i].tangentStep, &tangentNormals);
#ifdef FULL_DUMP

    dumpCloudA(folder + std::string("Tlabels") + std::to_string(iter) +
                   std::string("_") + std::to_string(i),
               i, model.scans[i].labels, &normLabel);
    dumpCloudA(folder + std::string("Tlabels_init") + std::to_string(iter) +
                   std::string("_") + std::to_string(i),
               i, model.scans[i].labels_initial);
    dumpCloudA(folder + std::string("Tlabels_partial") + std::to_string(iter) +
                   std::string("_") + std::to_string(i),
               i, model.scans[i].labels_partial);
    if (corpora.person_has_labels_2d[personId]) {
      for (int v = 0; v < ip.nviews; v++) {
        Matrix3X ray = model.scans[i].label2dRays[v];
        ray.colwise() -=
            model.scans[i].label2dRays[v].col(ip.njointlabels + ip.nverts);
        ray *= 10;

        dumpCloud(folder + std::string("Rays") + std::to_string(iter) +
                      std::string("_") + std::to_string(v) + "_",
  ...
  for (size_t i = 0; i < ip.njoints - 1; i++) {
...

    {
      ...
      for (size_t j = 0; j < vertex_ring.size(); j++) {
        size_t vertex_index = vertex_ring[j];
        Vector3 vp =
            s2v(inputData.readPoint3d(barriers.meanShapePrior + vertex_index));
        Matrix3X vv;
        vv.resize(3, 1);
    ...
    for(int c = 0; c < ip.ncomp; c++) {
        ...
        edge[1] = barriers.joints + ip.njoints*(c+1) + i + 1;
                ...
                for (size_t j = 0; j < vertex_ring.size(); j++) {
                        size_t vertex_index = vertex_ring[j];
            edge[2] = barriers.zero;
                        //auto& u = control_vertices.col(vertex_index);
    //.cast<Diffable>() edge[3] = barriers.shape + (c+1)*ip.nverts +
    vertex_index;
            //auto& v = global.meanShape.col(vertex_index);
                        //residual.col(r_idx) = factor / sqrt(vertex_ring.size()
    + global.joint_count)*((v - lv) - R*(u - lu)); //1*hyperparam
                        //r_idx++;
            terms[tid].add(edge);
                }
    }
