...
void InputData::addUnknown(Scalar3 _f3d) { f3d.push_back(_f3d); }

void InputData::addUnknown(Scalar2 _f2d) { f2d.push_back(_f2d); }

void InputData::addUnknown(Scalar _f1d) { f1d.push_back(_f1d); }

void InputData::addPoint(std::array<Scalar, LOCAL_SIZE> _dwd) {
  dwd.push_back(_dwd);
}

void InputData::addPoint(Scalar3 _d3d) { d3d.push_back(_d3d); }

void InputData::addPoint(Scalar2 _d2d) { d2d.push_back(_d2d); }

void InputData::addPoint(Scalar _d1d) { d1d.push_back(_d1d); }
...
void InputData::copy() {
  ...
  cudaMemcpy(cf3d, f3d.data(), f3d.size() * sizeof(Scalar3),
             cudaMemcpyHostToDevice);
  cudaMemcpy(cf2d, f2d.data(), f2d.size() * sizeof(Scalar2),
             cudaMemcpyHostToDevice);
  cudaMemcpy(cf1d, f1d.data(), f1d.size() * sizeof(Scalar),
             cudaMemcpyHostToDevice);

  assert(dwd[1][0] == dwd[0][LOCAL_SIZE]);

  cudaMemcpy(cdwd, dwd.data(), dwd.size() * sizeof(Scalar) * LOCAL_SIZE,
             cudaMemcpyHostToDevice);
  cudaMemcpy(cd3d, d3d.data(), d3d.size() * sizeof(Scalar3),
             cudaMemcpyHostToDevice);
  cudaMemcpy(cd2d, d2d.data(), d2d.size() * sizeof(Scalar2),
             cudaMemcpyHostToDevice);
  cudaMemcpy(cd1d, d1d.data(), d1d.size() * sizeof(Scalar),
             cudaMemcpyHostToDevice);
...
void InputData::retrieve() {
  ...
  cudaMemcpy(f3d.data(), cf3d, f3d.size() * sizeof(Scalar3),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(f2d.data(), cf2d, f2d.size() * sizeof(Scalar2),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(f1d.data(), cf1d, f1d.size() * sizeof(Scalar),
             cudaMemcpyDeviceToHost);

  cudaMemcpy(dwd.data(), cdwd, dwd.size() * sizeof(Scalar) * LOCAL_SIZE,
             cudaMemcpyDeviceToHost);
  cudaMemcpy(d1d.data(), cd1d, d1d.size() * sizeof(Scalar),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(d2d.data(), cd2d, d2d.size() * sizeof(Scalar2),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(d3d.data(), cd3d, d3d.size() * sizeof(Scalar3),
             cudaMemcpyDeviceToHost);
...
unsigned int InputData::dim() const {
  std::cout << "Size: " << f3d.size() << " " << f2d.size() << " " << f1d.size()
            << " " << dwd.size() << " " << d3d.size() << " " << d2d.size()
            << " " << d1d.size() << " " << std::endl;
...
void InputData::read(Matrix3X &A, BarrierIndex start, int size, bool backup) {
  ...
  if (!backup) {
    ...
    // std::cout << start << " " << size << " " << backup_f3d.size() <<
    // std::endl;
    for (int i = 0; i < size; i++) {
