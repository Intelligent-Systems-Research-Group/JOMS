...
int main(int argc, char *argv[]) {
  ...
  options.add_options()("o,out_path", "Output Directory",
      ...
      "j,jointly",
      "Iteration after which we jointly optimize in R^n and surface "
      "correspondences",
      cxxopts::value<int>()->default_value("180"))(
      "a,alternating",
      "Iteration interval after which to interleave discrete correspondences "
      "optimization",
      cxxopts::value<int>()->default_value("5"))("b,base_dataset_folder",
                                                 "path to dataset folder",
                                                 cxxopts::value<std::string>())(
      "u,use_model_to_data", "use model to data error term",
      cxxopts::value<bool>()->default_value("false")->implicit_value("true"))(
      "D,deform_joints", "number of muscle deform joints influence",
      cxxopts::value<int>())(
      "i,iteration_data", "Iteration after which the point cloud data is used",
      cxxopts::value<int>())("w,continuous_step_size", "Continuous step size",
                             cxxopts::value<float>())(
      "e,iteration_end", "Number of outer LM Iterations",
      cxxopts::value<int>()->default_value("999"))(
      "t,temporal_regularizer", "Smooth pose regularizer",
      cxxopts::value<bool>()->default_value("false")->implicit_value("true"))(
      "T,template_path", "path to template folder",
      cxxopts::value<std::string>()->default_value("template"))(
      "J,njoints", "Number of joints of the skeleton", cxxopts::value<int>())(
      "V,max_vertex_ring", "Max vertices on joint boundaries",
      cxxopts::value<int>())("C,center_vertex_id", "Center Vertex Id",
                             cxxopts::value<int>())(
      "S,enable_symmetry", "Enable symmetry",
      cxxopts::value<bool>()->default_value("false")->implicit_value("true"))(
  ...
  params.template_data_marker_path =
      params.template_path + "/markers_data2.txt";
  ...
  params.template_surface_map_path =
      params.template_path + "/surface_to_keypoints.json";

  params.use_symmetry = result["enable_symmetry"].as<bool>();
  std::map<std::string, std::vector<size_t>> marker_vertex_groups;
#ifdef USE_MARKERS
  // loadVertexGroups("markers_fixed.txt", &marker_vertex_groups, NULL, NULL);
  // //"markers.txt.old"
  loadVertexGroups(params.template_marker_path, &marker_vertex_groups, NULL,
                   NULL);
#endif
  //@TODO To config
  ...
  params.nlabels =
      marker_vertex_groups.size(); // marker_vertex_groups.size(); //100
  ...
  params.camPaths = {params.template_path + "/cameras.json",
                     params.template_path + "/caesar_cameras.json"};
  params.nviews = 8;
