...
struct OSD_Vertex {
  ...
  void Clear(void * = 0) { point.setZero(); }

  void AddWithWeight(OSD_Vertex const &src, float weight) {
    point += weight * src.point;
  }

  void SetPosition(float x, float y, float z) { point << x, y, z; }
...
struct SubdivEvaluator {
  ...
  void generate_refined_mesh(Matrix3X const &vert_coords, int levels,
                             MeshTopology *mesh_out, Matrix3X *verts_out);
  void init(Matrix3X const &vert_coords, Matrix3X *outS);
  // void move(const MeshTopology& mesh, const Matrix3X& vert_coords, const
  // Matrix2X& du, Matrix2X* dir);
  int increment_u_crossing_edges(const MeshTopology &mesh, Matrix3X const &X,
                                 int face, const Vector2 &u, const Vector2 &du,
                                 int *new_face_out, Vector2 *new_u_out,
                                 Vector2 *dir, Scalar *distance,
                                 std::vector<Vector3> *path = NULL);
  int increment_u_crossing_edges_simple(const MeshTopology &mesh,
  ...
  int increment_accurate(const MeshTopology &mesh, Matrix3X const &X, int face,
                         const Vector2 &u, const Vector2 &du, int *new_face_out,
                         Vector2 *new_u_out, Vector2 *dir, Scalar *distance,
                         std::vector<Vector3> *path = NULL);
  int minimizeSurfacePoint(const MeshTopology &mesh, SurfacePoint p,
                           const Matrix3X &X, const Matrix3X &target,
                           SurfacePoint &q, Matrix3X *pathCloud);
  ...
  void evaluateSubdivSurface(
      ...
      Matrix3X *out_S, triplets_t *out_dSdX = 0, triplets_t *out_dSudX = 0,
      triplets_t *out_dSvdX = 0, Matrix3X *out_Su = 0, Matrix3X *out_Sv = 0,
      Matrix3X *out_Suu = 0, Matrix3X *out_Suv = 0, Matrix3X *out_Svv = 0,
      Matrix3X *out_N = 0, Matrix3X *out_Nu = 0, Matrix3X *out_Nv = 0,
      bool printDebug = false) const;

  void evaluatePtex(const Matrix3X &vert_coords, Matrix3X *out_S,
                    triplets_t *out_dSdX = NULL, triplets_t *out_dSudX = NULL,
                    triplets_t *out_dSvdX = NULL, Matrix3X *out_Su = NULL,
                    Matrix3X *out_Sv = NULL, bool printDebug = false) const;
  ...
  Matrix3X evalPointsPartial(const Matrix3X &base,
                             const std::vector<SurfacePoint> &surfacePoints);
  void evalPointsPartial(const Matrix3X &localPts,
                         const std::vector<SurfacePoint> &surfacePoints,
                         Matrix3X *out_S, Matrix3X *out_Su = NULL,
  ...
  void createPatches(const Matrix3X &localPts, MeshTopology *mesh,
                     Matrix3X *verts);
};
