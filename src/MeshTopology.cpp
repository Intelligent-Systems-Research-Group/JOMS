...
bool saveObj(std::string path, const MeshTopology *mesh,
             const Matrix3X *verts) {
  std::filebuf fb;
...
bool loadObj(std::string path, MeshTopology *mesh, Matrix3X *verts,
             Matrix3X *normals, double scale, bool mirror, bool rot) {
  ...
  std::cout << "Verts: " << num_verts << " Normals: " << num_normals
            << std::endl;
  ...
  while (std::getline(infile, line)) {
    ...
    if (prefix == "v") {
      ...
      mesh->quads.col(face_idx) << vert_idxs[0], vert_idxs[1], vert_idxs[2],
          vert_idxs[3], vert_idxs[4];
      face_idx++;
    } else
      continue;
  }
...
void MeshTopology::vertexAdjacency(
    std::map<size_t, std::set<size_t>> &adjacency, IMatrix4X *faceAdj) {
  IMatrix2X edges;
...
void MeshTopology::initVertexToSurfaceCoordinate(int vidx, int &fidx, double &u,
                                                 double &v) {
  double e = 0.001;
  for (int i = 0; i < quads.cols(); i++) {
    for (int j = 0; j < 4; j++) {
...
void MeshTopology::vertexToSurfaceCoordinate(int vidx, int &fidx, double &u,
                                             double &v) {
...
MeshTopology::calculateLaplaceWeights() {
    ...
    for (size_t k = 0; k < (int) (nverts_per_face[f]); k++)
    {
      ...
      for (size_t fa = 0; fa < num_faces(); fa++)
      {
        ...
        for (size_t l = 0; l < (int)(nverts_per_face[fa]); l++)
          if ((quads(l, fa) == edge[1]) && (quads((l + 1) %
  (int)(nverts_per_face[fa]), fa) == edge[0])) { other = (int) fa; found++;
          }
      }
      assert(found == 1);
...
std::vector<double> dijkstra(const std::vector<std::vector<edge>> &graph,
                             int source) {
...
IMatrix4X
MeshTopology::findClosest(const Matrix3X &verts,
                          const std::vector<std::vector<int>> &vgroups,
                          const SparseMatrix &sparseWeights, Matrix4X *dist) {
...
void MeshTopology::computeMaskFaces(const std::vector<int> &invMaskedVertices,
...
const static Eigen::IOFormat XYZFormat(Eigen::StreamPrecision,
                                       Eigen::DontAlignCols, " ", "\n");
void dumpCloud(std::string name, int iter, const Matrix3X &cloud,
               const Matrix3X *normals) {
...
bool readCloud(std::string path, Matrix3X *cloud, Matrix3X *normals,
               double scale, bool mirror, bool rot) {
  std::ifstream infile(path);
  if (!infile.is_open()) {
    std::cout << "reading failed for: " << path << std::endl;
  } else {
...
void corruptNonUniform(Matrix3X *data, Matrix3X *normal) {
  ...
  Matrix3X result;
  result.resize(3, data->cols());
  Matrix3X resultNormal;
  resultNormal.resize(3, normal->cols());
  int j = 0;
  for (int i = 0; i < data->cols(); i++) {
