...
unsigned int RayTracer::addModel(RTCScene scene_i, const Matrix3X &V,
...
unsigned int RayTracer::addSubdivModel(RTCScene scene_i, const Matrix3X &V,
                                       const MeshTopology &top) {
  RTCGeometry mesh =
      rtcNewGeometry(device, RTC_GEOMETRY_TYPE_SUBDIVISION); // SUBDIVISION
  ...
  unsigned int *edgeCount = (unsigned *)rtcSetNewGeometryBuffer(
      mesh, RTC_BUFFER_TYPE_FACE, 0, RTC_FORMAT_UINT, sizeof(unsigned int),
      nfaces);
  ...
  unsigned int *indices = (unsigned int *)rtcSetNewGeometryBuffer(
      mesh, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT, sizeof(unsigned int),
      4 * nfaces);
...
void RayTracer::device_init(char *cfg, const Matrix3X &V,
                            const MeshTopology &top) {
  ...
  assert(rtcGetDeviceProperty(
             device, RTC_DEVICE_PROPERTY_SUBDIVISION_GEOMETRY_SUPPORTED) != 0);
...
float RayTracer::shoot(Vector3 p, Vector3 n, Vector3 dn, Vector2 *u, int *f) {
  ...
  float dist = 2 * range;
  // std::cout << "Ray: " << p.transpose() << " -> " << (-n).transpose() <<
  // std::endl;
  if (rayHit.hit.geomID != RTC_INVALID_GEOMETRY_ID) {
    if (rayHit.hit.Ng_x * dn[0] + rayHit.hit.Ng_y * dn[1] +
            rayHit.hit.Ng_z * dn[2] >
        0) {
      dist = rayHit.ray.tfar;
      *f = rayHit.hit.primID;
      (*u)(0, 0) = rayHit.hit.u;
      (*u)(1, 0) = rayHit.hit.v;
      if ((*u)(0, 0) >= 0.0 && (*u)(0, 0) <= 1.0 && (*u)(1, 0) >= 0.0 &&
          (*u)(1, 0) <= 1.0) {
      } else {
    ...
    // std::cout << "primID: " << rayHit.hit.primID << " -> " << rayHit.ray.tfar
    // << std::endl;
  } else {
...
Vector3 RayTracer::shootNormals(Vector3 p, Vector3 n, Vector2 *uout,
                                int *fout) {
