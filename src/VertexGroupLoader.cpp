...
void loadVertexGroups(std::string path, 
	...
	while (std::getline(infile, line))
	{
		...
		if (i == 1) {
			if (weights) (*weights)[JointVertexPair(border_names[0], index)] = 1.0;
		}
		if (i == 2) {
			if (weights) {
			...
			string first = border_names[0] < border_names[1] ? border_names[0] : border_names[1];
			string second = border_names[0] < border_names[1] ? border_names[1] : border_names[0];
			if(border_groups)
				(*border_groups)[std::pair<std::string, std::string>(first,second)].push_back(index);
		}
	}
}

std::map<std::pair<std::string, std::string>, Vector3> calculateJoints(const std::map<std::pair<std::string, std::string>, std::vector<size_t>>& vertex_groups, Matrix3X* vertices) {
	std::map<std::pair<std::string, std::string>, Vector3> joints;
	std::map<std::pair<std::string, std::string>, std::vector<size_t>>::const_iterator it;
...
std::map<std::pair<std::string, std::string>, VectorX> calculateInitialRegressor(const std::map<std::pair<std::string, std::string>, std::vector<size_t>>& vertex_groups, Matrix3X* vertices) {
